...
// creates a memento with current state
        ...
        System.out.println("[+] State saved. History size: " +
                history.size());
        System.out.println(history.size() == 1 ? "[+] Default State: "
    ...
    public void undo(ApplicationConfiguration appConfig) {
        if (history.size() > 1) {
            ...
            System.out.println("[+] Undo performed. History size: " +
                    history.size());
            System.out.println(history.size() == 1 ? "[+] Default State: " + mementoToBeRestored : "[+] Current State: " +
                    mementoToBeRestored);
        } else {
